...
class MLP:
    ...
    def __init__(self, batch_size, learning_rate, activation_func, hidden_layers, num_epoch, skip_epoch):
            ...
            for epoch in range(1, self.num_epoch + 1):
                ...
                try:
                    while True:
                        ...
                        sess.run([train_op], feed_dict={X: batch_X, y: batch_y, self.keep_prob: 0.8})
                except StopIteration:
                    pass
                train_acc = sess.run(accuracy, feed_dict={X: X_train, y: y_train, self.keep_prob: 1.0})
                test_acc = sess.run(accuracy, feed_dict={X: X_test, y: y_test, self.keep_prob: 1.0})
